...
async function command(msg: Discord.Message) {
    ...
    if (!data.periodic && data.portfolio) {
        await db.updatePartialData(data._id, {periodic: {state: false, interval: interval, lastUpdate: null}}, COLLECTIONS.CRYPTO);
    ...
    if (data && !data.periodic && !data.portfolio) {
        await db.updatePartialData(data._id, {periodic: {state: false, interval: interval, lastUpdate: null}}, COLLECTIONS.CRYPTO); //Create Empty Portfolio
